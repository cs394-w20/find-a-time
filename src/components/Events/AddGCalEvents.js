...
const AddGCalEvents = ({ roomId, user, events, startDate, endDate }) => {
  ...
  for (i = 0; i < events.length; i++) {
    if (i === 0) {
      intervalData = findIntervals(roomId, user.id, events[i])
    } else {
      _mergeRecursive(intervalData, findIntervals(roomId, user.id, events[i]))
    }
  }
  ...
  UpdateDb({ roomId, user, intervalData, updateType: "AUTO" })
}
...
const findIntervals = (roomId, userId, event) => {
  const start = getStartTime(event)
  const end = getEndTime(event)
  return findBuckets(roomId, userId, start, end)
}
